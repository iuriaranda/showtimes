...
test('get movies from lat/long', function (assert) {
  ...
  api.getMovies(function (err, movies) {
    assert.equal(err, null)
    assert.ok(movies.data.length > 1, 'more than one movie')
    assert.end()
  })
})
...
test('get movies from lat/long and get filtered movies for first movie name', function (assert) {
  ...
  api.getMovies(function (err, movies) {
    assert.equal(err, null)
    api.getMovies(movies.data[0].name, function (err2, movies2) {
      assert.equal(err, null)
      assert.ok(movies2.data.length > 0, 'movies found using query')
      assert.end()
    })
  })
})
...
test(
  ...
  function (assert) {
    ...
    api.getMovies(function (err, movies) {
      assert.equal(err, null)
      var query = movies.data[0].name
      api.getTheaters(query, function (err2, theaters) {
        assert.equal(err2, null)
        assert.ok(theaters.data.length === 0, 'no theaters')
        assert.end()
      })
    })
  }
